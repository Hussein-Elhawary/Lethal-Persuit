...
namespace our {
    ...
    class Entity {
        ...
        Entity() = default;
        // The entity constructor is private since only the world is allowed to instantiate an entity
    public:
        ...
        void deserializeBullet(const nlohmann::json &data);
        ...
        T *getComponent() {
            ...
            T *targetComponent = nullptr;
            for (auto component: components) {
                targetComponent = dynamic_cast<T *>(component);
        ...
        // This template method searches for a component of type T and deletes it
        void deleteComponent(const size_t index) {
            auto it = components.begin();
            std::advance(it, index);
            if (it != components.end()) {
                delete *it;
